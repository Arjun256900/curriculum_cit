...
router.get("/courses/category/all", async (req, res) => {
  try {
    const { department } = req.query;
    if (!department) {
      return res
        .status(400)
        .json({ message: "Missing department parameter" });
    }

    // List of regulations
    const regulations = ["R21", "R22", "R22R", "R24"];
    ...
    for (const regulation of regulations) {
      ...
      // Assign the data for this regulation
      allRegulationData[regulation] = categoryData;
    }

    res.json(allRegulationData); // Return the data grouped by regulation and category
  } catch (error) {
    console.error("Error fetching category-wise courses for all regulations:", error.message);
    res
      .status(500)
      .json({ message: "Error retrieving category-wise courses for all regulations" });
  }
});
